...
ideally with use of the [sshproxy script](https://docs.nersc.gov/connect/mfa/#sshproxy). While access to Perlmutter is then available via `ssh -i .ssh/nmdcda nmdcda@perlmutter.nersc.gov`, use VSCode's Remote Explorer is recommended for beginners. The following can be added to your SSH configuration file of choice:
