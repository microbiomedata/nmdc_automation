...
Things you'll need to get set up before you're able to run anything are as follows:
...
- access to the dev and prod [mongodb](https://docs.google.com/document/d/11h21epiEX2HVM8pUNYPw8dzx_9p0Rx8BkpHcDk62Utc/edit?tab=t.0#heading=h.qyxkoy9mt63k) tunnel to be accessed via MongoDB Compass 
- runtime api as you and nmdcda
...
- Remote Explorer setup on VSCode to nmdcda
  - While access to Perlmutter is then available via `ssh -i .ssh/nmdcda nmdcda@perlmutter.nersc.gov`, use VSCode's Remote Explorer is recommended for beginners. The following can be added to your SSH configuration file of choice:
    ...
    # Use private key generated by sshproxy.sh when accessing NERSC servers.
...
use the bash script
  replace bash script direct api call with db_tools.py release_jobs(config, id_list_file)
  check if it takes quoted, if not, 