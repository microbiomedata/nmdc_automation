...
def get_analysis_files_df(proposal_id: int, files_df: pd.DataFrame, ACCESS_TOKEN: str, remove_files: List[str]) -> pd.DataFrame:
    ...
    gold_analysis_files_df = gold_analysis_files_df[['jdp_file_id', 'apGoldId','studyId', 'itsApId', 'project_name', 
                                                     'biosample_id', 'seq_id', 'file_name', 'file_type', 'file_size', 'md5sum',
                                                     'analysis_project_id', 'seq_unit_name']]
    
    gold_analysis_files_df.columns = ['jdp_file_id', 'ap_gold_id', 'gold_study_id', 'its_ap_id', 'sequencing_project_name',
                                      'gold_biosample_id', 'gold_seq_id', 'file_name', 'file_type', 'jdp_file_size', 'md5sum','jgi_ap_id', 
                                      'seq_unit_name']
