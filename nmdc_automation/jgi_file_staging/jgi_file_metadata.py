...
def get_samples_data(
    samples_csv_file: str, proposal_id: int, project: str, config_file: str
) -> None:
    """
    ...
    all_files_list = get_sample_files(
        samples_csv_file, ACCESS_TOKEN, eval(config["JDP"]["delay"])
    )
    files_df = pd.DataFrame(all_files_list)

    gold_analysis_data = get_analysis_projects_from_proposal_id(
        proposal_id, ACCESS_TOKEN
    )
    gold_analysis_data_df = pd.DataFrame(gold_analysis_data)
    gold_analysis_files_df = pd.merge(
        gold_analysis_data_df,
        files_df,
        left_on="itsApId",
        right_on="analysis_project_id",
    )
    gold_analysis_files_df = remove_unneeded_files(
        gold_analysis_files_df, eval(config["JDP"]["remove_files"])
    )
...
def get_access_token() -> str:
    ...
    sys.exit(
        f"get_access_token: {response.text}"
...
def check_access_token(ACCESS_TOKEN: str, delay: float) -> str:
    gold_biosample_url = (
        "https://gold-ws.jgi.doe.gov/api/v1/projects?biosampleGoldId=Gb0291582"
    )
    headers = {
        "Authorization": f"Bearer {ACCESS_TOKEN}",
        "accept": ACCEPT,
        "User-agent": "nmdc bot 0.1",
    }
    time.sleep(delay)
...
def get_sample_files(
    samples_csv_file: str, ACCESS_TOKEN: str, delay: float
) -> List[dict]:
    """
    ...
    for idx, biosample_id in samples_df.itertuples():
        ...
        combine_sample_ids_with_agg_ids(
            sample_files_list, agg_id_list, biosample_id, seq_id, all_files_list
        )
...
def get_sequence_id(gold_id: str, ACCESS_TOKEN: str, delay: float):
    ...
    gold_biosample_url = (
        f"https://gold-ws.jgi.doe.gov/api/v1/projects?biosampleGoldId={gold_id}"
    )
    headers = {
        "Authorization": f"Bearer {ACCESS_TOKEN}",
        "accept": ACCEPT,
...
def get_analysis_projects_from_proposal_id(
    proposal_id: int, ACCESS_TOKEN: str
) -> List[dict]:
    ...
    ap_type_gold_analysis_data = [
        proj
        for proj in gold_analysis_data
        if proj["apType"] in ["Metagenome Analysis", "Metatranscriptome Analysis"]
    ]
...
def combine_sample_ids_with_agg_ids(
    sample_files_list, agg_id_list, biosample_id, seq_id, all_files_list
) -> None:
    for sample, agg_id in zip(sample_files_list, agg_id_list):
        for files_dict in sample:
            seq_unit_name = (
                files_dict["metadata"]["seq_unit_name"]
                if "seq_unit_name" in files_dict["metadata"].keys()
                else None
            )
            file_format = (
                files_dict["metadata"]["file_format"]
                if "file_format" in files_dict["metadata"].keys()
                else None
            )
            ...
            all_files_list.append(
                {
                    ...
                    "seq_id": seq_id,
                    "file_name": files_dict["file_name"],
                    "file_status": files_dict["file_status"],
                    "file_size": files_dict["file_size"],
                    "jdp_file_id": files_dict["_id"],
                    "md5sum": md5sum,
...
def remove_unneeded_files(
    seq_files_df: pd.DataFrame, remove_files_list: list
) -> pd.DataFrame:
    """
...
def remove_large_files(
    ...
    for file_type in remove_files_list:
        seq_files_df = seq_files_df[
            (~seq_files_df["file_name"].str.contains(file_type))
        ]
    return seq_files_df
...
def remove_duplicate_analysis_files(seq_files_df: pd.DataFrame) -> pd.DataFrame:
    ...
    for gold_id in ap_gold_ids:
        ...
        for idx, row in seq_files_df.loc[
            seq_files_df.apGoldId == gold_id, :
        ].iterrows():
            # find rows with fastq files to remove (fastq file name is not in list of seq_unit_names and is not
            # input.corr.fastq.gz)
            if (
                "fastq" in row.file_name
                and ~np.any([seq in row.file_name for seq in seq_unit_names_list])
                and row.file_name != "input.corr.fastq.gz"
            ):
                drop_idx.append(idx)
...
def insert_samples_into_mongodb(sample_list: list, mdb) -> None:
    ...
    try:
        ...
        for d in sample_list:
            db_records_list.append(
                {
                    key: value
                    for (key, value) in d.items()
                    if key in list(Sample.__fields__.keys())
                }
            )
